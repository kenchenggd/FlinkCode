package com.atguigu.flink.flinkCEP

import com.atguigu.flink.flinkCEP.FlinkCEPExample.LoginEvent
...
object FlinkCEPExample {
  ...
  case class  LoginEvent(userId:String,ip:String,
                         eventType: String,
                         eventTime:String)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val stream = env
      .fromElements(
          ...
          LoginEvent("2","192.168.0.4","fail","4")
      )
    ...
    val loginFailStream = patternStream
      .select((pattern:Map[String,Iterable[LoginEvent]]) => {
        ...
        val thrid = pattern.getOrElse("thrid",null).iterator.next()

        "userId是： " + first.userId + " 的用户， 10s 之内连续登录失败了三次，" + "ip地址分别是： " + first.ip + "; " + second.ip + "; " + thrid.ip
      })
